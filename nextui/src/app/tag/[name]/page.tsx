...
export default function TagDetailPage() {
  ...
  const tagName = useMemo(() => decodeURIComponent(params?.name || ""), [params?.name]);
  ...
  useEffect(() => {
    const fetchTagArticles = async () => {
      ...
      setError(null);
      try {
        const res = await apiFetch(`/api/article/tag?tag=${encodeURIComponent(tagName)}`);
        ...
        if (json.code === 200) {
          ...
          throw new Error(json.msg || "Error fetching tag articles");
        }
      } catch (e) {
    ...
    <div className="min-h-screen bg-background flex flex-col items-center pt-24 overflow-x-hidden">
      <div className="w-full max-w-6xl mx-auto px-6 py-12 flex-grow flex flex-col">
        <div className="mb-10">
          ...
          <p className="text-white/60">Articles under this tag.</p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center text-white/50 py-12">No articles found for this tag.</div>
        ) : (
          <div className="space-y-4">
