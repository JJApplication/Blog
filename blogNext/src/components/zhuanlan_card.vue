<template>
  <div class="zhuanlan-card">
    ...
    <el-dialog title="新建专栏" :visible.sync="show" width="80%" :before-close="handleClose">
    ...
    <el-table :data="zlist" max-height="calc(100vh - 180px)" border style="width: 100%; overflow-y: auto">
      <el-table-column prop="primary_id" label="ID" sortable width="80"></el-table-column>
      <el-table-column label="别名" sortable width="160">
        <template slot-scope="scope">
          <el-input v-model="scope.row.name" placeholder="请输入内容"></el-input>
        </template>
      </el-table-column>
      <el-table-column sortable width="200" label="标题">
        <template slot-scope="scope">
          <el-input v-model="scope.row.title" placeholder="请输入内容"></el-input>
        </template>
      </el-table-column>
      <el-table-column width="400px" label="列表">
        <template slot-scope="scope">
          <el-input
            type="textarea"
      ...
      <el-table-column label="操作" width="150">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="handleUpdate(scope.row, scope.row.primary_id)">更新</el-button>
          <el-button size="mini" type="danger" @click="handleDelete(scope.row, scope.row.primary_id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
