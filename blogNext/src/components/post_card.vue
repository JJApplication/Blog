<template>
  <div class="post-card">
    ...
    <div id="post-list">
      <el-table id="post-list-inner" :data="postData" border style="width: 100%">
        <el-table-column prop="id" label="ID" sortable width="80"></el-table-column>
        <el-table-column prop="name" label="URI" sortable width="200"></el-table-column>
        <el-table-column prop="title" sortable label="标题"></el-table-column>
        <el-table-column label="操作" width="280">
          <template slot-scope="scope">
            <el-button size="mini" type="success" @click="handleOpen(scope.$index, scope.row.name)">编辑</el-button>
            <el-button size="mini" @click="handleEdit(scope.$index, scope.row.name)">打开</el-button>
            <el-button size="mini" type="primary" @click="export_post(scope.row.name)">导出</el-button>
            <el-button size="mini" type="danger" @click="handleDelete(scope.$index, scope.row.name)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <el-dialog title="文章更新" :visible.sync="post_edit" width="80%">
    ...
    <el-dialog title="文章编辑" :visible.sync="post_open" width="94%">
      <el-row :gutter="20">
        ...
        <el-col :span="16">
          <el-input v-model="post_open_tags" style="">
            <template slot="prepend">标签</template>
          </el-input>
        </el-col>
      </el-row>
    ...
    <el-dialog title="新增文章" :visible.sync="add_post_open" width="70%">
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input style="color: #404040; font-weight: bold" v-model="add_post_name" placeholder="必填">
            <template slot="prepend">文件名</template>
          </el-input>
        </el-col>
        <el-col :span="16">
          <el-input v-model="get_date" style="">
            <template slot="prepend">日期</template>
          </el-input>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input style="color: #404040; font-weight: bold" v-model="add_post_title">
            <template slot="prepend">标题</template>
          </el-input>
        </el-col>
        <el-col :span="16">
          <el-input v-model="add_post_tags" style="">
            <template slot="prepend">标签</template>
          </el-input>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    upload_file(params) {
      ...
      this.$http
        .post(api_dash.post + '?type=file&name=' + this.name, form, {
          headers: {
