<template>
  <div class="zhuanlan">
    ...
    <div class="zhuanlan_body">
      <div
        style="border: 4px solid #2f2f2f; width: fit-content; padding: 20px 16px; margin: 0 auto"
      ...
      >
        <h3 id="title">
          ...
          <br />
          <span
            style="font-size: 1rem; font-weight: normal; font-family: Monaco, Consolas, 'DejaVu Sans Mono', monospace"
          >
            renj.io
          </span>
        </h3>
        ...
        <el-row :gutter="10">
          <el-col :xs="12" :sm="8" :md="6" :lg="6" :xl="6" v-for="z in zhuanlan_list" :key="z.link">
            <div class="zhuanlan-tab animated fadeInDown">
              <h4>
                <a :href="'/zhuanlan/' + z.link">{{ z.title }}</a>
              </h4>
...
export default {
  ...
  methods: {
    get_zhuanlan_list() {
      this.$http
        .get(api_zhuanlan.api_zhuanlan_list)
        .then((res) => {
          if (res.data.data) {
        ...
        .catch((e) => {
          this.zhuanlan_list = [this.empty_zhuanlan]
        })
    },
  },
}
