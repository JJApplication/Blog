<template>
  <div class="overview">
    ...
    <div style="max-width: 960px; margin: 20px auto 0">
      ...
      <el-dialog title="缓存测试" :visible.sync="dialogVisible" class="dg">
        <span>测试文章pin</span>
        <p>
          <span>缓存测试结果:</span>
        ...
        <p>
          自定义测试: /api/sys/checkredis?name=
          <span style="color: #4bff67">文章地址</span>
        </p>
...
export default {
  ...
  methods: {
    test() {
      this.$http
        .get(api_statistic.api_statistic_check, { params: { name: 'pin' } })
        .then((res) => {
          this.hit = res.data
