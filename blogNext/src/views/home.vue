<template>
  <div class="home">
    ...
    <el-drawer title="🔨" :visible.sync="drawer" direction="ltr" size="64%" :modal="false" :with-header="false">
      <div class="drawer-content">
        <div class="img">
          ...
          <div class="me">
            <p style="font-weight: bold">
              ...
              <span class="link">{{ custom.author }}</span>
            </p>
            <p style="font-weight: bold">
              <span class="label">My site</span>
              <a class="link" :href="custom.site_url">{{ custom.site_name }}</a>
            </p>
            <p style="font-weight: bold">
              <span class="label">Github</span>
              <a class="link" :href="'https://github.com/' + custom.github">{{ custom.github }}</a>
            </p>
          </div>
          <div class="small-bt">
          ...
          <el-collapse>
            <el-collapse-item title="我的项目" name="1">
              <div style="text-align: left">
                {{ custom.project_des }}
                <a :href="custom.project_url">{{ custom.project }}</a>
              </div>
            </el-collapse-item>
          </el-collapse>
        </div>
        <div class="more">
          ...
          <el-tooltip content="查看站点总览" placement="bottom">
            <i
              class="el-icon-info"
              style="margin-top: 15px; cursor: pointer; font-size: 14px; color: #9f9f9f; font-weight: bold"
              @click="overview"
            >
              OVERVIEW
            </i>
          </el-tooltip>
          <p style="color: #9f9f9f; text-align: left; padding-left: 10px; margin-top: 15px; font-size: 14px">
            博客文章的版权归作者所有，转载时请注明来源
          </p>
        </div>
      </div>
    </el-drawer>
      ...
      <div class="contents">
        <div class="right animated slideInRight">
          ...
          <div class="more">
            ...
            <p style="color: #9f9f9f; text-align: left; padding-left: 10px; margin-top: 10px; font-size: 14px">
              博客文章的版权归作者所有，转载时请注明来源
            </p>
          </div>
        </div>
    ...
    <el-dialog title="博客状态" :visible.sync="dialogVisible" class="sys">
      <div style="width: 100%; margin: 0 auto">
        <span style="width: 120px; display: inline-block; margin-right: 10px">当前的routine协程</span>
        <el-input v-model="routine" :disabled="true" style="margin-bottom: 10px; max-width: 140px"></el-input>
        <br />
        <span style="width: 120px; display: inline-block; margin-right: 10px">本站的访问量</span>
        <el-input v-model="uv" :disabled="true" style="max-width: 140px; margin-bottom: 10px"></el-input>
        <br />
        <span style="width: 120px; display: inline-block; margin-right: 10px">当天的请求次数</span>
        <el-input v-model="count" :disabled="true" style="max-width: 140px; margin-bottom: 10px"></el-input>
        <br />
        <span style="width: 120px; display: inline-block; margin-right: 10px">站点运行时长</span>
        <el-input v-model="days" :disabled="true" style="max-width: 140px; margin-bottom: 10px"></el-input>
      </div>
    ...
    <el-dialog id="user-login" title="登录到后台" :visible.sync="login" width="50%">
...
export default {
  ...
  methods: {
    ...
    status() {
      ...
      let _this = this
      this.$http
        .get(api_statistic.api_statistic_views)
        .then((res) => {
    ...
    open(type) {
      if (type === 'mail') {
        ...
        let text =
          '<strong>觉得不错可以赞助我！</strong><br>' +
          '<img style="max-width: 140px;margin-top: 10px;vertical-align: middle" src="' +
          pay +
          '">' +
...
@media (max-width: 360px) {
  .pagenation /deep/ .el-pagination .btn-prev,
  .pagenation /deep/ .el-pagination .btn-next,
  .pagenation /deep/ .el-pagination .el-pager li {
    min-width: 24px;
  }
  .sys /deep/ .el-dialog {
    width: 95%;
...
.more /deep/ .el-button:focus,
.more /deep/ .el-button:hover {
  border-color: var(--button-border);
}
...
.home /deep/ .el-collapse-item__wrap {
  background-color: var(--post-background);
  border-color: var(--border-color);
  color: var(--text-color);
}
